...
class P2p(CreateTracker):
    ...
    @classmethod
    def from_json(cls, p2p_rub_usdt_json: Dict, p2p_usdt_lkr_json: Dict, p2p_uah_usdt_json: Dict, p2p_eur_usdt_json: Dict, p2p_usd_usdt_json: Dict):
        ...
        p2p, _ = cls.objects.update_or_create(timestamp=timestamp, defaults=p2p_obj)
        print(p2p, _)
        return
    ...
    def pay_trade_history() -> Dict:
        ...
        session.headers.update(
            {"Content-Type": "application/json;charset=utf-8", "X-MBX-APIKEY": BINANCE_API}
        )
        print(url)
...
class Pairs(models.Model):
    ...
    @classmethod
    def get_obj(cls) -> List:
        pairs = cls.objects.all().values('id','pair','ru_pair')
        return list(pairs)
...
class Terms(models.Model):
    ...
    @classmethod
    def get_dict(cls) -> Dict:
        terms = list(cls.objects.all().values('terms_of_use_user','terms_of_use_merchant', 'size_fee'))
        dict_terms = {}
        for el in terms:
...
class Order(CreateUpdateTracker):
    client_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='client_id_order_set')
    city = models.CharField(max_length=32, **nb)
    pair = models.CharField(max_length=32, **nb)
    summ = models.FloatField(default=0)
    period = models.CharField(max_length=32, **nb)
    ...
    merchant_executor_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='merchant_executor_id_order_set', **nb)
...
class Suggestion(CreateUpdateTracker):
    order_id = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='order_id_suggestion_set')
    merchant_executor_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='merchant_executor_id_suggestion_set', **nb)
    summ = models.FloatField(default=0)

class Rating(CreateUpdateTracker):
    who_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='who_id_rating_set')
    whom_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='whom_id_rating_set')
    rank = models.IntegerField(blank=False, null=True)
    comment = models.TextField(**nb)

class MerchantsCities(models.Model):
    merchant_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='merchant_id_cities_set', **nb)
    city_id = models.ForeignKey(Cities, on_delete=models.CASCADE, related_name='city_merchant_ids_set', **nb)
